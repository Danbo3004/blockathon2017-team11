...
let candidates = {};
let candidateName = {1 : 'Minh', 2 : 'Chien', 3 : 'Tuan', 4: 'Nga'};
...
window.voteForCandidate = function() {
  var searchIDs = $("#candidate_id input:checkbox:checked").map(function(){
    return $(this).val();
  }).get();
  let candidateIds = searchIDs;
  Voting.deployed().then(function(contractInstance) {
    contractInstance.voteForCandidate(eventId, candidateIds, {gas: 4600000, from: web3.eth.accounts[0]}).then(function() {
      for (let i = 0; i< candidateIds.length ; i++) {
        contractInstance.totalVotesFor.call(candidateIds[i]).then(function(v) {
          $("#" + prefixDivId + candidateIds[i]).html(v.toString());
          updateTotalVoteByEventId(eventId);
          $("#msg").html("");
        });
      }
      $("#candidate_id input:checkbox:checked").map(function(){
